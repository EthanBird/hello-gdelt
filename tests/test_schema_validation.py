...
from hello_gdelt.gdelt.schema import validate_tsv_file, validate_tsv_sample
...
def test_file_validation_streams_rows(tmp_path: Path) -> None:
    path = tmp_path / "events.tsv"
    path.write_text(row(61) + "\n" + row(61), encoding="utf-8")
    result = validate_tsv_file("events", path, max_rows=1)
    assert result.passed
